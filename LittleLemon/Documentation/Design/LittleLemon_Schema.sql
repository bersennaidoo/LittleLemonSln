...
CREATE INDEX `MENU_ID_idx` ON `LittleLemonDB`.`CUSTOMER` (`MENU_ID` ASC) VISIBLE;
...
CREATE INDEX `FK_CUS_ID_idx` ON `LittleLemonDB`.`CONTACT` (`CUS_ID` ASC) VISIBLE;
...
CREATE UNIQUE INDEX `CUS_ID_UNIQUE` ON `LittleLemonDB`.`ADDRESS` (`CUS_ID` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `LittleLemonDB`.`ORDERSTATUS` (
  ...
  CONSTRAINT `FK_ORDERSTATUS_ORD_ID`
    FOREIGN KEY (`ORD_ID`)
